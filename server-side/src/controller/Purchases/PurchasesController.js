...
exports.CreatePurchases= async (req,res)=>{
    let response = await CreateParentChildService(req,ParentModel,ChildsModel,'PurchaseID');
...
exports.PurchasesDelete=async (req, res) => {
    let response = await DeleteParentChildsService(req,ParentModel,ChildsModel,'PurchaseID');
    res.status(200).json(response)
}
//
