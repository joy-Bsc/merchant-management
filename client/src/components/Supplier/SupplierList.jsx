...
import { SupplierDetailsRequest, SupplierListRequest, DeleteSupplier, } from "../../APIRequest/SupplierAPIRequest";
...
const SupplierList = () => {
  ...
  useEffect(() => {
    SupplierListRequest(1, perPageKey, searchKey);
  }, [searchKey, perPageKey]);

  let ALLProduct = useSelector((state) => state.supplier.List);
  let Total = useSelector((state) => state.supplier.ListTotal);

  const handlePageClick = (event) => {
    SupplierListRequest(event.selected + 1, perPageKey, searchKey);
  };
  ...
  const PageKeyOnChange = (e) => {
    ...
    SupplierListRequest(1, e.target.value, searchKey);
  };
  ...
  const searchOnChange = (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKey("0");
      SupplierListRequest(1, perPageKey, "0");
    }
  };
  const handleShowModal = async (id) => {
    ...
    try {
      const details = await SupplierDetailsRequest(id);
      console.log("Supplier Details:", details);
  ...
  const DeleteItem = async(id) => {
    ...
    if (Result) {
     let DeleteResult = await DeleteSupplier(id);
      if(DeleteResult){
        SupplierListRequest(1, perPageKey, searchKey);
      }
    }
