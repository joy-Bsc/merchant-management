...
import { CategoriesDetailsRequest, CategoriesListRequest, DeleteCategories, } from "../../APIRequest/CategoryAPIRequest";
...
const CategoryList = () => {
  ...
  useEffect(() => {
    CategoriesListRequest(1, perPageKey, searchKey);
  }, [searchKey, perPageKey]);

  let ALLProduct = useSelector((state) => state.category.List);
  let Total = useSelector((state) => state.category.ListTotal);

  const handlePageClick = (event) => {
    CategoriesListRequest(event.selected + 1, perPageKey, searchKey);
  };
  ...
  const PageKeyOnChange = (e) => {
    ...
    CategoriesListRequest(1, e.target.value, searchKey);
  };
  ...
  const searchOnChange = (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKey("0");
      CategoriesListRequest(1, perPageKey, "0");
    }
  };
  const handleShowModal = async (id) => {
    ...
    try {
      const details = await CategoriesDetailsRequest(id);
  ...
  const DeleteItem = async(id) => {
    ...
    if (Result) {
     let DeleteResult = await DeleteCategories(id);
      if(DeleteResult){
        CategoriesListRequest(1, perPageKey, searchKey);
      }
    }
