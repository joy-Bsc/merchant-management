...
import { CustomerDetails, CustomerListRequest, DeleteCustomer, } from "../../APIRequest/CustomerAPIRequest";
...
const CustomerList = () => {
  ...
  useEffect(() => {
    CustomerListRequest(1, perPageKey, searchKey);
  }, [searchKey, perPageKey]);

  let ALLProduct = useSelector((state) => state.customer.List);
  let Total = useSelector((state) => state.customer.ListTotal);

  const handlePageClick = (event) => {
    CustomerListRequest(event.selected + 1, perPageKey, searchKey);
  };
  ...
  const PageKeyOnChange = (e) => {
    ...
    CustomerListRequest(1, e.target.value, searchKey);
  };
  ...
  const searchOnChange = (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKey("0");
      CustomerListRequest(1, perPageKey, "0");
    }
  };
  const handleShowModal = async (id) => {
    ...
    try {
      const details = await CustomerDetails(id);
  ...
  const DeleteItem = async(id) => {
    ...
    if (Result) {
     let DeleteResult = await DeleteCustomer(id);
      if(DeleteResult){
        CustomerListRequest(1, perPageKey, searchKey);
      }
    }
