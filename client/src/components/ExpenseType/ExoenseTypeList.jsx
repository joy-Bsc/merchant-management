...
import { ExpenseTypeDetailsRequest, ExpenseTypeRequest, DeleteExpenseType, } from "../../APIRequest/ExpenseTypeAPIRequest";
...
const ExpenseTypeList = () => {
  ...
  const [ExpenseTypeDetails, setExpenseTypeDetails] = useState(null);

  useEffect(() => {
    ExpenseTypeRequest(1, perPageKey, searchKey);
  }, [searchKey, perPageKey]);

  let ALLProduct = useSelector((state) => state.expensetype.List);
  let Total = useSelector((state) => state.expensetype.ListTotal);

  const handlePageClick = (event) => {
    ExpenseTypeRequest(event.selected + 1, perPageKey, searchKey);
  };
  ...
  const PageKeyOnChange = (e) => {
    ...
    ExpenseTypeRequest(1, e.target.value, searchKey);
  };
  ...
  const searchOnChange = (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKey("0");
      ExpenseTypeRequest(1, perPageKey, "0");
    }
  };
  const handleShowModal = async (id) => {
    ...
    try {
      const details = await ExpenseTypeDetailsRequest(id);
      console.log("ExpenseType Details:", details);
      
      setExpenseTypeDetails(details);
      setShowModal(true);
    } catch (error) {
      console.error("Failed to fetch ExpenseType details:", error);
    }
  };

  const handleCloseModal = () => {
    setShowModal(false);
    setExpenseTypeDetails(null);
  };

  const DeleteItem = async(id) => {
    let Result = await DeleteAlert();
    if (Result) {
     let DeleteResult = await DeleteExpenseType(id);
      if(DeleteResult){
        ExpenseTypeRequest(1, perPageKey, searchKey);
      }
    }
                                  ...
                                  <ExpenseTypeDetailsModal
                                       show={showModal}
                                         handleClose={handleCloseModal}
                                          ExpenseTypeDetails={ExpenseTypeDetails}
                                         />
