    ...
    try {
        ...
        if (result.status === 200 && result.data.status === "success") {
            ...
            if (Rows.length > 0) {
                ...
                store.dispatch(SetCustomerListTotal(Total[0].count));
            } else {
...
export async function CreateCustomerRequest(PostBody, ObjectID) {
    ...
    if (ObjectID && ObjectID !== 0) {
        ...
        console.log("Updating customer with ID:", ObjectID);
    } else {
        URL = `${BaseURL}/CreateCustomers`;
...
export async function CustomerDetails(ObjectID) {
    let URL = `${BaseURL}/CustomersDetailsByID/${ObjectID}`;
    store.dispatch(ShowLoader());
    try {
        const result = await axios.get(URL, AxiosHeader);
